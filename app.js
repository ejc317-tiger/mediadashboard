const viewConfig = {
  companies:{eyebrow:'PRIVATE MARKETS & AI',title:'Company intelligence',subtitle:'Track ownership, investors, sector, transaction history, valuations, and business profiles.',filterLabel:'All sectors',headers:[['name','Company'],['category','Sector'],['ownership','Ownership'],['investorsLabel','VC investors'],['status','Status'],['metric','Latest disclosed metric'],['date','As of']]},
  vcfirms:{eyebrow:'VENTURE CAPITAL',title:'VC firm portfolios',subtitle:'Search venture investors and cross-reference their portfolio companies against the AI company database.',filterLabel:'All firm types',headers:[['name','VC firm'],['category','Strategy'],['headquarters','Headquarters'],['portfolio_count','AI companies']]},
...
function normalize(record){const normalized=Object.fromEntries(Object.entries(record).map(([key,value])=>[fieldAliases[key]||key,value]));normalized.investorsLabel=(normalized.investors||[]).map(investor=>investor.name).join(', ')||'—';return normalized;}
...
function openDrawer(record){
  ...
  const relationships=currentView==='vcfirms'?(record.portfolio||[]).map(company=>`<li><span class="company-logo">${safe(company.name[0])}</span><div><strong>${safe(company.name)}</strong><small>${safe(company.category)} · ${safe(company.round_name)}</small></div></li>`).join(''):(record.investors||[]).map(investor=>`<li><span class="company-logo">${safe(investor.name[0])}</span><div><strong>${safe(investor.name)}</strong><small>${safe(investor.round_name)}${Number(investor.is_lead)?' · Lead investor':''}</small></div></li>`).join('');
  const relationshipTitle=currentView==='vcfirms'?'AI portfolio companies':'VC investors';
