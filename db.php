...
function hasDatabaseConfiguration(): bool
{
    if (trim((string) (getenv('DB_NAME') ?: '')) !== '' && trim((string) (getenv('DB_USER') ?: '')) !== '') return true;
    if (!is_file(__DIR__ . '/config.php')) return false;
    try {
        $config = require __DIR__ . '/config.php';
        return is_array($config) && trim((string) ($config['database'] ?? '')) !== '' && trim((string) ($config['username'] ?? '')) !== '';
    } catch (Throwable) {
        return true; // A present but invalid file must not reopen unauthenticated setup.
    }
}

/** Validate and save initial database settings without placing them in tracked source. */
function saveDatabaseConfiguration(array $input): void
{
    if (hasDatabaseConfiguration()) throw new RuntimeException('Database configuration already exists and cannot be replaced from the public setup screen.');
    $config = [
        'host' => trim((string) ($input['host'] ?? 'localhost')) ?: 'localhost',
        'port' => trim((string) ($input['port'] ?? '3306')) ?: '3306',
        'database' => trim((string) ($input['database'] ?? '')),
        'username' => trim((string) ($input['username'] ?? '')),
        'password' => (string) ($input['password'] ?? ''),
    ];
    if ($config['database'] === '' || $config['username'] === '') throw new InvalidArgumentException('Database name and username are required.');
    if (!ctype_digit($config['port']) || (int) $config['port'] < 1 || (int) $config['port'] > 65535) throw new InvalidArgumentException('Enter a valid database port.');
    ...
    try {
        ...
        throw new RuntimeException('Could not connect with those database settings. Check the values and try again.', 0, $exception);
    }
    $contents = "<?php\n/** Generated by the Northstar setup screen. This file is not tracked by git. */\nreturn " . var_export($config, true) . ";\n";
    $temporary = tempnam(__DIR__, '.database-config-');
    if ($temporary === false || file_put_contents($temporary, $contents, LOCK_EX) === false || !rename($temporary, __DIR__ . '/config.php')) {
        if (is_string($temporary) && is_file($temporary)) @unlink($temporary);
        throw new RuntimeException('Database settings could not be saved. Check application directory write permissions.');
    }
...
function requiredColumnDefinitions(): array
{
    return [
        ...
        'app_settings'=>['setting_key'=>'VARCHAR(100) NULL','setting_value'=>'TEXT NULL','updated_by'=>'BIGINT UNSIGNED NULL','updated_at'=>'TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'],
        'research_jobs'=>['id'=>'CHAR(48) NULL','user_id'=>'BIGINT UNSIGNED NULL','response_id'=>'VARCHAR(100) NULL','prompt'=>'LONGTEXT NULL','model'=>'VARCHAR(100) NULL','depth'=>'VARCHAR(20) NULL','maximum_records'=>'INT UNSIGNED NULL','providers_json'=>'TEXT NULL','status'=>"VARCHAR(30) NOT NULL DEFAULT 'Queued'",'report'=>'LONGTEXT NULL','result_json'=>'LONGTEXT NULL','error_message'=>'VARCHAR(1000) NULL','created_at'=>'TIMESTAMP DEFAULT CURRENT_TIMESTAMP','updated_at'=>'TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP','completed_at'=>'TIMESTAMP NULL'],
        ...
        'private_equity_firms'=>['name'=>'VARCHAR(180) NULL','strategy'=>"VARCHAR(120) NOT NULL DEFAULT 'Private Equity'",'headquarters'=>'VARCHAR(180) NULL','aum'=>'DECIMAL(20,2) NULL','aum_currency'=>'CHAR(3) NULL','key_contacts'=>'TEXT NULL','description'=>'TEXT NULL','source_name'=>'VARCHAR(180) NULL','source_url'=>'VARCHAR(2048) NULL','as_of_date'=>'DATE NULL','confidence'=>"VARCHAR(20) NOT NULL DEFAULT 'Review'",'updated_at'=>'TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'],
        'vc_firms'=>['name'=>'VARCHAR(180) NULL','category'=>"VARCHAR(100) NOT NULL DEFAULT 'Venture Capital'",'headquarters'=>'VARCHAR(180) NULL','aum'=>'DECIMAL(20,2) NULL','aum_currency'=>'CHAR(3) NULL','key_contacts'=>'TEXT NULL','description'=>'TEXT NULL','source_name'=>'VARCHAR(180) NULL','source_url'=>'VARCHAR(2048) NULL','as_of_date'=>'DATE NULL','confidence'=>"VARCHAR(20) NOT NULL DEFAULT 'Review'",'updated_at'=>'TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'],
